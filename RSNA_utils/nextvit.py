...
class NextViTSupervision(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
                 strides=[1, 2, 2, 2], sr_ratios=[8, 4, 2, 1], head_dim=32, mix_block_ratio=0.75,
                 use_checkpoint=False):
        super(NextViTSupervision, self).__init__()
        self.use_checkpoint = use_checkpoint
        ...
        self.sup_inds = [11, 16, 21, 26]
        self.sv1 = nn.Sequential(nn.BatchNorm2d(512, eps=NORM_EPS),
                                 nn.AdaptiveAvgPool2d((1, 1)),
                                 nn.Flatten(),
                                 nn.Linear(512, 1))

        self.sv2 = nn.Sequential(nn.BatchNorm2d(512, eps=NORM_EPS),
                                 nn.AdaptiveAvgPool2d((1, 1)),
                                 nn.Flatten(),
                                 nn.Linear(512, 1))

        self.sv3 = nn.Sequential(nn.BatchNorm2d(512, eps=NORM_EPS),
                                 nn.AdaptiveAvgPool2d((1, 1)),
                                 nn.Flatten(),
                                 nn.Linear(512, 1))

        self.sv4 = nn.Sequential(nn.BatchNorm2d(512, eps=NORM_EPS),
                                 nn.AdaptiveAvgPool2d((1, 1)),
                                 nn.Flatten(),
                                 nn.Linear(512, 1))
...
class NextViTSupervisionSmall(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
                 strides=[1, 2, 2, 2], sr_ratios=[8, 4, 2, 1], head_dim=32, mix_block_ratio=0.75,
                 use_checkpoint=False):
        super(NextViTSupervisionSmall, self).__init__()
        self.use_checkpoint = use_checkpoint
        ...
        self.sup_inds = [6, 11, 16]
        self.sv1 = nn.Sequential(nn.BatchNorm2d(256, eps=NORM_EPS),
                                 nn.AdaptiveAvgPool2d((1, 1)),
                                 nn.Flatten(),
                                 nn.Linear(256, 1))
